...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 14 22:47:18 2021
...
The SVRG part has been annoted as this goes beyond the topic of report.
...
from method_SGD import *

tf.disable_v2_behavior()

## USAGE
# python experiment_2.py -d mnist -a 1234 -so 2345 -b 250 -ne 50
## In the report I use, for example,
...
random_switch   = prog_option["Random"]
...
if random_switch != 0:
    ...
    rand_std   = 0.4
    if random_switch == 1:
        x_test = x_test + 255.0 * rand_scale * np.random.normal(rand_mean, rand_std, x_test.shape)
        x_test[x_test>255.0] = 255.0
        x_test[x_test<0.0] = 0.0
    elif random_switch == 2:
        x_train = x_train + 255.0 * rand_scale * np.random.normal(rand_mean, rand_std, x_train.shape)
...
logits, w_list, = model2(x, input_size, output_size)
...
if batch_size == 1:
    ...
    # ProxSPDB
    if data_name == 'mnist':
        eta_prox_spdb = 0.016 # mnist
    elif data_name == 'fashion_mnist':
        eta_prox_spdb = 0.005 # fashion_mnist
    elif data_name == 'cifar':
        eta_prox_spdb = 0.0006 # fashion_mnist
    ...
    # ProxSGD, shared with SGD
    eta_prox_sgd = 0.008
    eta_prime_prox_sgd = 0.08#0.05
    prox_sgd_batch_size = batch_size
    
    # SGD
    eta_sgd = 0.001
    eta_prime_sgd = 0.01#0.05
...
# SGD 
if (alg_list["SGD"]):
    w_sgd, hist_NumGrad_sgd, hist_NumEpoch_sgd, hist_TrainLoss_sgd, \
     hist_TrainAcc_sgd, hist_TestAcc_sgd \
        = SGD(x, y, x_train, y_train, x_test, y_test, \
        sgd_batch_size, eta_sgd, eta_prime_sgd, eta_comp, lamb, max_num_epoch,\
        w_list, loss_operation, accuracy_operation, verbose, log_enable)
...
#change the name of x-axis to epoch
...
#=================================================================
    ...
    ##Print training and testing accuracy, choose largest in last 10 epoch
    print_len = max(max_num_epochs, 10)
    if (alg_list["ProxSARAH"] and prox_sarah_option['1']):
        print('ProxSARAH - single')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_prox_sarah1[-1:-1-print_len:-1]), max(hist_TestAcc_prox_sarah1[-1:-1-print_len:-1]))
            )

    if (alg_list["ProxSARAH"] and prox_sarah_option['2']):
        print('ProxSARAH - b=sqrt(n), gamma = 0.95')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_prox_sarah2[-1:-1-print_len:-1]), max(hist_TestAcc_prox_sarah2[-1:-1-print_len:-1]))
            )

    if (alg_list["ProxSARAH"] and prox_sarah_option['3']):
        print('ProxSARAH - b=sqrt(n), gamma = 0.99')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_prox_sarah3[-1:-1-print_len:-1]), max(hist_TestAcc_prox_sarah3[-1:-1-print_len:-1]))
            )

    if (alg_list["ProxSARAH"] and prox_sarah_option['4']):
        print('ProxSARAH - ProxSARAH b=n^(1/3), gamma = 0.95')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_prox_sarah4[-1:-1-print_len:-1]), max(hist_TestAcc_prox_sarah4[-1:-1-print_len:-1]))
            )

    if (alg_list["ProxSARAH"] and prox_sarah_option['5']):
        print('ProxSARAH - b=n^(1/3), gamma = 0.99')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_prox_sarah5[-1:-1-print_len:-1]), max(hist_TestAcc_prox_sarah5[-1:-1-print_len:-1]))
            ) 

    if (alg_list["ProxSpiderBoost"]):
        print('ProxSpiderBoost')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_prox_spdb[-1:-1-print_len:-1]), max(hist_TestAcc_prox_spdb[-1:-1-print_len:-1]))
            )

    if (alg_list["ProxSGD"]):
        print('ProxSGD')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_prox_sgd[-1:-1-print_len:-1]), max(hist_TestAcc_prox_sgd[-1:-1-print_len:-1]))
            )
        
    if (alg_list["SGD"]):
        print('SGD')
        print('best train accuarcy: {}, best test accuarcy: {}'.format(
            max(hist_TrainAcc_sgd[-1:-1-print_len:-1]), max(hist_TestAcc_sgd[-1:-1-print_len:-1]))
            )
